...
int main() {
  ...
  while (1) {
    ...
    auto const swidth = std::min(draw::string_width(gc32, s), draw::ordinate{128});
    draw_string(frame_buffer, gc32, s, point{static_cast<draw::ordinate>(128 - swidth), -1});

    wmove(stdscr, 0, 0);

    auto xb = 0U;  // The x ordinate (in bytes)
    auto yb = 0U;
    for (auto const d : frame_buffer.store()) {
      for (auto bit = 0; bit < 8; ++bit) {
        mvprintw(yb, (xb * 8U) + bit, "%c", (d & (0x80_b >> bit)) != 0_b ? '*' : ' ');
      }
      ++xb;
      if (xb >= frame_buffer.stride()) {
